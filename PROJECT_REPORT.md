...
The pipeline is designed using a modular, decoupled architecture consisting of six core components:

```mermaid
graph TD
    A[LiDAR sensor /kitti/velo/pointcloud] -->|PointCloud2| B(LidarFilter)
    C[Camera sensor /kitti/cam2/image_raw] -->|Image| D(YoloDetector)
    IMU[IMU sensor /kitti/oxts/imu] -->|Imu msg| ODOM(OdometryEstimator)
    ...
    B -->|Raw Points| ODOM
    ODOM -->|Estimated Pose| E
    ODOM -->|Estimated Odom| PUB_ODOM[Publisher /fusion/estimated_odom]
    
    E -->|Detections & Points| J(OccupancyNetwork)
    J -->|3D Voxel Grid| K[Publisher /fusion/occupancy_grid]
    J -->|3D Object & Ego Boxes| L[Publisher /fusion/detected_objects_3d]
    
    E -->|Projected Voxels + Overlays| M[Publisher /fusion/bev_image]
...
3.  **3D Occupancy Network Module (`occupancy_network.py`):** Generates 3D voxel grids representing the background and target volumes, overlays oriented 3D bounding boxes for obstacles, and manages rendering of the red ego vehicle marker at `base_link`.
4.  **Lidar-IMU Odometry Estimator (`odometry_estimator.py`):** Uses raw IMU inputs to construct a dynamic motion prediction (initial guess) and Open3D's ICP Scan Matching to register consecutive LiDAR scans, tracking vehicle translation, orientation, and path in real-time.
5.  **Geometric Projection Module (`projection_module.py`):** Manages coordinate transformations. It performs rigid-body 3D transforms (LiDAR to camera), camera rectification, and 3D-to-2D projection using camera projection matrices and camera models (pinhole/fisheye). It also handles occlusion filtering via a custom Z-buffer algorithm.
6.  **Sensor Fusion Node (`fusion_node.py`):** Acts as the central ROS node. It synchronizes image and point cloud streams using an `ApproximateTimeSynchronizer`, invokes the processing modules, performs spatial data association, estimates 3D centroids, and publishes the fused output messages, 2D BEV Top View overlays, and visualization streams.
...
### 6.4. 2D/3D BEV Projection Sync & Overhead Minimization
Updating the 2D BEV canvas by separately converting point clouds and finding voxels caused significant processing overhead, adding ~700ms of delay.
*   **Decision:** Refactored the callback execution to perform point cloud conversion (`cloud_to_numpy`) and invalid point filtering exactly once. Stored the 3D voxel grid centroids and colors as cached instance variables in the `OccupancyNetwork`, allowing the 2D BEV image generator to project them directly onto the 2D canvas in a single pass.
*   **Result:** Resolved spatial discrepancies between 2D and 3D views and reduced the total node latency from ~1500ms down to ~800ms.

### 6.5. TF Synchronization (`use_sim_time`)
Playing recorded datasets in a loop caused frame clock synchronization conflicts, triggering `TF_OLD_DATA` and `TF_REPEATED_DATA` warnings.
*   **Decision:** Enabled `<param name="/use_sim_time" value="true"/>` in the ROS launch file to force all nodes to utilize the simulated clock generated by the `rosbag` player.

### 6.6. Lidar-IMU Odometry Fusion & ICP Initialization
Pure IMU double-integration is highly prone to rapid drift due to accelerometer noise and gravity bias, while LiDAR-only scan matching can get stuck in local minima (especially in highway/corridor-like environments).
*   **Decision:** Implemented a prediction-correction filter. High-rate IMU readings are integrated to compute a relative translation and rotation prediction (motion model initial guess). When a LiDAR frame arrives, the point cloud is voxel downsampled and registered against the previous frame using Open3D's ICP, initialized with the IMU guess. The resulting transform updates the global pose, and a failure fallback uses the IMU prediction if ICP fitness drops below a threshold.
...
*   **3D Occupancy Voxel Grid & Bounding Box Marker Generation:** $\sim 3.4\text{ms}$ per frame.
*   **Lidar-IMU Odometry Estimation (Voxel + Open3D ICP):** $\sim 22.0\text{ms}$ per frame.
*   **2D BEV Top View Voxel Projection:** $\sim 1.5\text{ms}$ per frame.
*   **Total Callback Processing Latency (CPU-only):** $\sim 820\text{-}920\text{ms}$ per frame.
*   **Publishing Frequency:** Stable execution at the maximum sensor rate matching the bag clock.
